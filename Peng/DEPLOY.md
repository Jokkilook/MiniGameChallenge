...
- **주소: <https://pung-vs.fly.dev/>**
- 앱 `pung-vs` · 리전 `nrt`(도쿄) · 머신 1개 · `shared-cpu-1x` / 256MB
...
flyctl scale count 1 -a pung-vs     # 2개면 이걸로 줄임
```
...
```bash
curl -s -o /dev/null -w "%{http_code}\n" https://pung-vs.fly.dev/                  # 200
curl -s -o /dev/null -w "%{http_code}\n" https://pung-vs.fly.dev/levels/_all.js    # 200
curl -s -X POST https://pung-vs.fly.dev/__save -d '{}' -w " [%{http_code}]\n"      # 403
curl -s -X POST https://pung-vs.fly.dev/__bake -d '{}' -w " [%{http_code}]\n"      # 403
...
curl -s -i -N --max-time 8 -H "Connection: Upgrade" -H "Upgrade: websocket" \
  ...
  "https://pung-vs.fly.dev/ws?room=testroom" | head -1
```
...
- 앱 상태·로그·머신: <https://fly.io/apps/pung-vs> — 여기는 **금액이 아니라 메트릭**만 봅니다
- CLI 로 앱 페이지 열기: `flyctl dashboard -a pung-vs`
...
```bash
flyctl apps destroy pung-vs
```

**마감선: 2026년 8월 하순** (2026-08-08 기준 심사 기간 약 2주).
잠깐 멈추기만 할 거면 `flyctl scale count 0 -a pung-vs`.
