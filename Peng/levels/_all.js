...
PENG.defineLevel('peak', {
  ...
  collapse: {
    ...
    y: -0.5, hy: 0.5,
    col: '#4f8a45',                  // 고원 잔디
    platCol: '#8a8f5c',              // 정상 대지 — 풀이 벗겨진 마른 흙
    warnCol: '#e0713a',
    first: 16,                       // 첫 붕괴까지(초)
    every: 4.2,                      // 고리 한 겹이 무너지는 간격(초)
    warn: 1.5,                       // 무너지기 전 깜빡이는 예고(초)
    keepRings: 2                     // 정상(고리 0~1)은 끝까지 남는다
  },
  boxes: [
    // 저 아래 골짜기 — 떨어졌다는 게 눈으로 확인돼야 한다. 절벽면은 생성기가 만든다
    {cx:0, cy:-70, cz:0, hx:150, hy:1, hz:150, col:'#2b3a2c', deco:true}
  ],
  goal: {cx:0, cy:-999, cz:0, r:0.1},
  checkpoints: [ {x:0, y:0, z:0} ],
  killY: -26,
