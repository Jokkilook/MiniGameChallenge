...
PUNG.PHYS = {
  ...
  /* 사격 쿨다운(doFire 의 0.12초) 중에 누른 클릭을 기억하는 시간(초) — JUMP_BUF 의
     사격판이다. 이게 없을 때는 쿨다운 중에 들어온 클릭이 그냥 버려졌다. 탄창이 3발
     (AMMO_MAX)이라 세 발을 빠르게 쏟아붓는 것이 기본 동작인데, 앞 발과 0.12초 안에
     겹친 클릭은 소리도 반동도 없이 사라져 "씹혔다"가 된다.
     연사 속도는 그대로 0.12초에 묶여 있다 — 달라지는 것은 누른 것이 버려지느냐,
     풀리는 순간 대신 나가느냐 하나뿐이다. 값을 JUMP_BUF 와 같게 둔 것은 두 유예가
     같은 뜻이기 때문이다: 한 박자 일찍 누른 것은 누른 것으로 친다.
     아래 BLAST_JUMP_GRACE 주석이 말하는 "사격에는 대응물이 없어 생긴 비대칭"이
     이것이었다. */
  FIRE_BUF:0.12,
...
PUNG.genPeak = function(seed){
  ...
  /* 위 겹의 윗면 높이는 두 조건 사이에서 골라야 한다.
       · 데크 윗면(-0.02처럼 붙이면): 충돌 없는 바위 윗면이 '밟을 수 있는 땅'으로
         보인다 — 걸어 들어가면 메시 속으로 가라앉아 화면이 흙색으로 가득 찬다.
       · 슬래브 밑면 아래(-0.30으로 내리면): 밖에서 볼 때 데크와 바위 사이에
         틈이 생겨 판이 기둥 위에 뜬 팬케이크처럼 보인다(실측: 둘 다 겪었다).
     슬래브 두께가 0.24~0.30유닛이라, -0.20 이면 바위가 슬래브 옆구리에 계속
     물려 있어 틈이 없고, 윗면은 걷는 평면보다 0.6m 아래에 온다.

     높이만으로는 부족하다 — 눈높이 1.6m 에서 10m 밖을 보면 0.6m 단차는 시야각
     3° 라 거의 안 보이고, 모래빛 바위(rock-sand)는 슬래브 베벨과 같은 황토색이라
     바닥이 그대로 이어지는 것처럼 읽혔다. 그래서 위 겹은 '회색' 바위만 쓴다:
     이 맵의 색 어휘에서 회색은 장애물·절벽이고 황토·갈색이 바닥이다. */
  ...
  /* 벼랑 끝 바위 — 데크 가장자리를 따라. 엄폐도 되고 절벽 입술도 된다.
     회색만 쓴다(림 위 겹과 같은 이유): 모래빛 바위는 슬래브 베벨·흙바닥과 같은
